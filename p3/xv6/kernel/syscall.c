...
{  
  ...
  if(((addr >= p->sz) || (addr + 4 >= p->sz)) && ((addr <= p->stackBoundary) || (addr >= USERTOP))) {
      return -1;
  }
...
fetchstr(struct proc *p, uint addr, char **pp)
{
  ...
  if((addr > p->sz) && (addr <= p->stackBoundary)) {
    return -1;
  }
  *pp = (char*)addr;
  ep = (char*)p->sz;

  if(addr < p->stackBoundary){
    ep = (char*)p->sz;
  } 
  else {
    ep = (char*)USERTOP;
...
argptr(int n, char **pp, int size)
{
  ...
  if((uint)i >= proc->sz || (uint)i+size > proc->sz) {
    if((uint)i <= proc->stackBoundary || (uint)i + size >= USERTOP) {
      return -1;
    }
  }
  *pp = (char*)i;
